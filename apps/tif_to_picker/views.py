...
def is_color_fill_layer(layer):
    ...
    return any(hasattr(item, 'key') and item.key == PsdKey.SOLID_COLOR_SHEET_SETTING 
              for item in layer.info)
def extract_layers(file_path, output_dir, output_format='PNG', quality=100, display_max_dimension=2700, optimize=True):
    """
    ...
    if file_size_mb > 50:
        display_max_dimension = display_max_dimension * 0.7
    elif file_size_mb > 100:
        display_max_dimension = display_max_dimension * 0.7
    elif file_size_mb > 200:
        display_max_dimension = display_max_dimension * 0.7
    
    print(f"🔍 EXTRACT_LAYERS: Max display dimension: {display_max_dimension * 0.7}")
