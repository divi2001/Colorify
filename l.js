...
async function processPallet(layerCtx, totalLayers, layerCanvas, currentLayerIndex, oldColorArray, trendingColors, paletteIndex) {
    ...
    try {
        if (totalLayers === 1) {
            ...
            // Create array of color mappings - IMPORTANT: map by button index
            const colorMappings = [];
            for (let i = 0; i < unlockedButtons.length; i++) {
                ...
                // Use button index to determine target color index
                // This ensures 1:1 mapping by position
                const targetColorIndex = (i + paletteIndex) % trendingColors.length;
            ...
            // Use direct color mapping for consistent palette replacement
            const processedImageUrl = await colorProcessor.applyDirectColorMapping(
                imageDataUrl,
                colorMappings
            );
            ...
            // Multi-layer processing with direct mapping
            const colorButtons = document.querySelectorAll('.color-picker');
            ...
            for (let layer = 1; layer <= totalLayers; layer++) {
                ...
                // Get target color by index to maintain 1:1 relationship
                const targetColorIndex = (layer - 1 + (paletteIndex || 0)) % trendingColors.length;
                const targetColor = trendingColors[targetColorIndex];
            ...
            // Process all layers in parallel with direct mapping
            const layerPromises = unlockedLayers.map(async ({ layer, canvas, button }) => {
                ...
                // Use direct color mapping instead of complex family-based mapping
                const processedImageUrl = await colorProcessor.applyDirectColorMapping(
                    imageDataUrl,
                    [mapping]
                );
...
colorFamilies.forEach(family => {
    ...
    if (bestDistance < threshold) {
        ...
        family.mappingIndex = -1;
            ...
            // Step 6: Enhanced smoothing for gradients
            console.log("Smoothing gradients...");
            
            // Check if there are any gradients to smooth
            ...
            if (hasGradients) {
                ...
                const kernelSize = 5; // Increased kernel size for better smoothing
                const kernelRadius = Math.floor(kernelSize / 2);
                
                // Process all pixels, not just gradient-marked ones
                for (let y = kernelRadius; y < height - kernelRadius; y++) {
                    for (let x = kernelRadius; x < width - kernelRadius; x++) {
                        const pixelIndex = y * width + x;
                        const idx = pixelIndex * 4;
                        
                        // Skip transparent or absolute black pixels
                        if (data[idx + 3] < 128 || (data[idx] <= 3 && data[idx + 1] <= 3 && data[idx + 2] <= 3)) {
                            continue;
                        }
                        
                        // Determine adaptive smoothing strength
                        // Apply stronger smoothing to gradient pixels, but some to all pixels
                        let blendStrength = gradientMap[pixelIndex] ? 
                            0.5 + gradientStrengthMap[pixelIndex] * 0.5 : 0.15;
                        
                        // Apply weighted smoothing with Gaussian kernel
                        ...
                        for (let dy = -kernelRadius; dy <= kernelRadius; dy++) {
                            for (let dx = -kernelRadius; dx <= kernelRadius; dx++) {
                                ...
                                const weight = Math.exp(-distance * distance / (2 * kernelRadius));
                ...
                // Apply smoothed values with blend factor based on gradient status
                for (let i = 0; i < data.length; i += 4) {
                    if (data[i + 3] >= 128 && !(data[i] <= 3 && data[i + 1] <= 3 && data[i + 2] <= 3)) {
                        const pixelIndex = Math.floor(i / 4);
                        const blendFactor = gradientMap[pixelIndex] ? 1.0 : 0.7;
                        
                        data[i] = Math.round(tempData[i] * blendFactor + data[i] * (1 - blendFactor));
                        data[i + 1] = Math.round(tempData[i + 1] * blendFactor + data[i + 1] * (1 - blendFactor));
                        data[i + 2] = Math.round(tempData[i + 2] * blendFactor + data[i + 2] * (1 - blendFactor));
                    }
                }
            }
