...
function displayColorPalette(layerIndex, colors, collection = 'trending', maxVisibleColors = 6, totalLayers = 1) {
    ...
    // Helper function to generate varied colors
    ...
    function generateVariedColors(baseColors) {
      // Generate a set of random colors
      const colors = [];
      for(let i = 0; i < 20; i++) {  // generating 8 random colors
    ...
    if (totalLayers === 1) {
      // Generate 3 trending palettes for single layer
      for (let i = 0; i < 10; i++) {
        ...
        // Generate different variations for each palette
        let adjustedColors = generateVariedColors([...colors]);
        
        // Add more color variations
        const extraColors = [];
        for (let j = 0; j < 3; j++) {
          const baseColor = adjustedColors[Math.floor(Math.random() * adjustedColors.length)];
          const variation = baseColor.map(component => 
            Math.min(255, Math.max(0, component + Math.floor(Math.random() * 60 - 30)))
          );
          extraColors.push(variation);
        }
        
        adjustedColors = [...adjustedColors, ...extraColors];
        ...
        window.originalPaletteColors[`${prefix}${i}`] = [...colors];
  ...
  async applyMultipleColorMappings(imageData, colorMappings) {
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            // Calculate DeltaE 2000 for better perceptual color difference
            function deltaE2000(lab1, lab2) {
                ...
                // Calculate a'1, a'2 (adjusted for better perceptual uniformity)
                const G = 0.5 * (1 - Math.sqrt(Math.pow(Cbar, 7) / (Math.pow(Cbar, 7) + Math.pow(25, 7))));
                const a1prime = a1 * (1 + G);
                const a2prime = a2 * (1 + G);
                
                // Calculate C'1, C'2, and h'1, h'2
                const C1prime = Math.sqrt(a1prime * a1prime + b1 * b1);
                const C2prime = Math.sqrt(a2prime * a2prime + b2 * b2);
                
                // Get hue angles, handling edge cases
                ...
                const RT = -2 * Math.sqrt(Math.pow(Cprimebar, 7) / (Math.pow(Cprimebar, 7) + Math.pow(25, 7))) 
                         * Math.sin((60 * Math.exp(-1 * Math.pow((hbarPrime - 275) / 25, 2))) * Math.PI / 180);
            ...
            // Identify gradients in the image
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 1; x < width; x++) {
                    ...
                    // Use color difference in Lab space for gradient detection
                    const lab1 = rgbToLab(r1, g1, b1);
                    const lab2 = rgbToLab(r2, g2, b2);
                    const diff = deltaE2000(lab1, lab2);
                    
                    // Small but noticeable differences indicate gradients
                    if (diff > 0.5 && diff < 10) {
                        gradientMap[y * width + x - 1] = true;
                        gradientMap[y * width + x] = true;
                    }
                }
            }
            
            // Detect vertical gradients
            for (let x = 0; x < width; x++) {
                for (let y = 1; y < height; y++) {
                    ...
                    // Use color difference in Lab space
                    const lab1 = rgbToLab(r1, g1, b1);
                    const lab2 = rgbToLab(r2, g2, b2);
                    const diff = deltaE2000(lab1, lab2);
                    
                    if (diff > 0.5 && diff < 10) {
                        gradientMap[(y - 1) * width + x] = true;
                        gradientMap[y * width + x] = true;
                    }
                }
            }
            
            // Prepare color clusters upfront for all colors
            const colorClusters = new Map();
            
            // Create more granular clusters based on color space position
            // Adjust quantization levels based on whether pixels are in gradient areas
            for (let i = 0; i < data.length; i += 4) {
                ...
                // Use different quantization for gradient vs. non-gradient areas
                const hueStep = isGradient ? 5 : 10;  // Finer granularity for gradients
                const satStep = isGradient ? 5 : 10;
                const valStep = isGradient ? 5 : 10;
                
                // Create cluster key based on quantized values
                const clusterKey = [
                    Math.round(pixelHsv[0] / hueStep) * hueStep,
                    Math.round(pixelHsv[1] / satStep) * satStep,
                    Math.round(pixelHsv[2] / valStep) * valStep,
                    isGradient ? 'g' : 'f'  // Mark as gradient or flat
                ].join(',');
                ...
                cluster.sumRGB[1] += g;
                cluster.sumRGB[2] += b;
                // Store pixel index for faster access
                cluster.pixels.push(i);
            }

            // Determine which clusters match each source color with better handling for gradients
            const clusterTransformations = new Map();
            
            // For each color mapping, find matching clusters
            colorMappings.forEach(mapping => {
                ...
                // For each cluster, check if it matches the original color
                colorClusters.forEach((cluster, clusterKey) => {
                    // Get average cluster color for pure black detection
                    const avgR = Math.round(cluster.sumRGB[0] / cluster.count);
                    const avgG = Math.round(cluster.sumRGB[1] / cluster.count);
                    const avgB = Math.round(cluster.sumRGB[2] / cluster.count);
                    
                    // Check if this is pure black (very low RGB values)
                    const isPureBlack = avgR <= 5 && avgG <= 5 && avgB <= 5;
                    
                    // Skip transformation for pure black
                    if (isPureBlack) {
                        return; // Skip this cluster entirely
                    }
                    
                    // Use better color difference metric
                    const labDistance = deltaE2000(cluster.lab, originalLab);
                    
                    // Calculate hue difference with wrap-around handling
                    let hueDiff = Math.abs(cluster.hsv[0] - originalHsv[0]);
                    if (hueDiff > 180) hueDiff = 360 - hueDiff;
                    
                    // Different thresholds for gradient vs flat colors
                    const labThreshold = cluster.isGradient ? 35 : 25;
                    const hueThreshold = cluster.isGradient ? 45 : 30;
                    
                    // Determine if cluster should be transformed
                    let shouldTransform = false;
                    
                    // Special handling for achromatic colors (grays and white)
                    if (originalHsv[1] < 15 || cluster.hsv[1] < 15) {
                        // For white or near-white colors
                        if (cluster.lab[0] > 85) { // High L value indicates white/very light color
                            shouldTransform = true;
                        }
                        // For gray shades
                        else if (cluster.hsv[1] < 10 && cluster.hsv[2] > 10 && cluster.hsv[2] < 95) {
                            shouldTransform = true;
                        }
                        // For other achromatic colors, focus more on lightness similarity
                        else {
                            shouldTransform = Math.abs(cluster.lab[0] - originalLab[0]) < 30;
                        }
                    } else {
                        // For chromatic colors, use multiple criteria
                        shouldTransform = (
                            labDistance < labThreshold || // Similar in Lab space
                            hueDiff < hueThreshold || // Similar hue
                            // Special case for similar lightness but different hue
                            (Math.abs(cluster.lab[0] - originalLab[0]) < 10 && hueDiff < 90)
                        );
                    }
                    
                    if (shouldTransform) {
                        // Calculate transformation parameters with improvements
                        let hueDiff = targetHsv[0] - originalHsv[0];
                        // Handle hue wrap-around cases
                        if (hueDiff > 180) hueDiff -= 360;
                        else if (hueDiff < -180) hueDiff += 360;
                        
                        // Calculate ratios with protection against division by zero
                        const satRatio = originalHsv[1] < 1 ? 
                            (targetHsv[1] < 1 ? 1 : targetHsv[1]) : 
                            targetHsv[1] / originalHsv[1];
                            
                        const valRatio = originalHsv[2] < 1 ? 
                            (targetHsv[2] < 1 ? 1 : targetHsv[2]) : 
                            targetHsv[2] / originalHsv[2];
                        
                        // Better strength calculation based on perceptual difference
                        const strength = cluster.isGradient ? 
                            Math.max(0.1, 1 - (labDistance / 50)) : // Gentler for gradients
                            Math.max(0.3, 1 - (labDistance / 100)); // Stronger for flat areas
                        
                        clusterTransformations.set(clusterKey, {
                            ...
                            valRatio,
                            strength,
                            isGradient: cluster.isGradient
                        });
                    }
                });
            });
            
            // Apply transformations with better gradient handling
            clusterTransformations.forEach((transform, clusterKey) => {
                const cluster = colorClusters.get(clusterKey);
                if (!cluster) return;
                
                // Get average cluster color for more accurate transformation
                const avgR = Math.round(cluster.sumRGB[0] / cluster.count);
                const avgG = Math.round(cluster.sumRGB[1] / cluster.count);
                const avgB = Math.round(cluster.sumRGB[2] / cluster.count);
                
                // Skip if this is pure black
                if (avgR <= 5 && avgG <= 5 && avgB <= 5) {
                    return;
                }
                
                const avgHsv = rgbToHsv(avgR, avgG, avgB);
                
                cluster.pixels.forEach(pixelIndex => {
                    const r = data[pixelIndex];
                    const g = data[pixelIndex + 1];
                    const b = data[pixelIndex + 2];
                    
                    // Skip pure black pixels entirely
                    if (r <= 5 && g <= 5 && b <= 5) {
                        return;
                    }
                    ...
                    // Determine transformation approach based on whether this is a gradient
                    if (transform.isGradient) {
                        // For gradients: preserve relative relationships between colors
                        // Calculate how current pixel relates to cluster average
                        const hueOffset = ((pixelHsv[0] - avgHsv[0] + 360) % 360);
                        const hueDelta = hueOffset > 180 ? hueOffset - 360 : hueOffset;
                        
                        const satRatio = avgHsv[1] > 0 ? pixelHsv[1] / avgHsv[1] : 1;
                        const valRatio = avgHsv[2] > 0 ? pixelHsv[2] / avgHsv[2] : 1;
                        
                        // Apply transformation while preserving relative values
                        let targetHue = (transform.target[0] + hueDelta) % 360;
                        if (targetHue < 0) targetHue += 360;
                        
                        // Adjust transformation strength based on how close pixel is to gradient edge
                        let newHue = (pixelHsv[0] + transform.hueDiff * transform.strength) % 360;
                        if (newHue < 0) newHue += 360;
                        
                        // Use scaled ratios to preserve gradient relationships
                        let newSat = Math.max(0, Math.min(100,
                            pixelHsv[1] * Math.pow(transform.satRatio, transform.strength)
                        ));
                        
                        let newVal = Math.max(0, Math.min(100,
                            pixelHsv[2] * Math.pow(transform.valRatio, transform.strength)
                        ));
                        
                        // Convert back to RGB
                        const newRGB = hsvToRgb(newHue, newSat, newVal);
                        
                        data[pixelIndex] = newRGB[0];
                        data[pixelIndex + 1] = newRGB[1];
                        data[pixelIndex + 2] = newRGB[2];
                    } else {
                        // For flat areas: direct transformation
                        let newHue = (pixelHsv[0] + transform.hueDiff * transform.strength) % 360;
                        if (newHue < 0) newHue += 360;
            ...
            // Final pass: smooth any harsh transitions that might occur between transformed regions
            if (true) { // You can make this conditional if needed
                const tempData = new Uint8ClampedArray(data);
                
                // Only smooth pixels near boundaries between different transformations
                for (let y = 1; y < height - 1; y++) {
                    for (let x = 1; x < width - 1; x++) {
                        const idx = (y * width + x) * 4;
                        ...
                        // Check if this pixel is at a transformation boundary
                        const isBoundary = (() => {
                            ...
                            for (let dy = -1; dy <= 1; dy++) {
                                for (let dx = -1; dx <= 1; dx++) {
                                    ...
                                    // Skip out of bounds or transparent neighbors
                                    if (nx < 0 || nx >= width || ny < 0 || ny >= height || data[nidx + 3] < 128) {
                                        continue;
                                    }
                                    
                                    // Compare colors
                                    const colorDiff = Math.abs(data[idx] - data[nidx]) + 
                                                      Math.abs(data[idx + 1] - data[nidx + 1]) + 
                                                      Math.abs(data[idx + 2] - data[nidx + 2]);
                                    
                                    // If significant difference, it's a boundary
                                    if (colorDiff > 40) {
                                        return true;
                                    }
                                }
                        ...
                        if (isBoundary && gradientMap[y * width + x]) {
                            ...
                            // Apply weighted average (75% original, 25% neighbors)
                            tempData[idx] = Math.round(0.75 * data[idx] + 0.25 * (rSum / count));
                            tempData[idx + 1] = Math.round(0.75 * data[idx + 1] + 0.25 * (gSum / count));
                            tempData[idx + 2] = Math.round(0.75 * data[idx + 2] + 0.25 * (bSum / count));
                        }
                    }
                }
        ...
        img{{ forloop.counter }}.onload = function() {
          ...
          ctx{{ forloop.counter }}.drawImage(img{{ forloop.counter }}, 0, 0);
            ...
            if(totalLayers === 1) {
                ...
                colorCount = 15;  // Get more colors for single layer
          ...
          setTimeout(() => {
            ...
            if (totalLayers === 1) {
                ...
                for (let i = 0; i < 10; i++) {
                    displayColorPalette(i, rgbColors, 'trending', distinctColorsArrayColorPallet.length+1, totalLayers);
                    displayColorPalette(i, rgbColors, 'ss', distinctColorsArrayColorPallet.length+1, totalLayers);
                    displayColorPalette(i, rgbColors, 'aw', distinctColorsArrayColorPallet.length+1, totalLayers);
                }
            } else {
        ...
        if (loadedImagesCount === totalLayers) {
            ...
            setTimeout(() => {
                ...
                if (layerCanvas) {
                    ...
                    displayColorPalette(layerIndex, paletteColors,'trending');
                    displayColorPalette(layerIndex, paletteColors,'ss');
                    displayColorPalette(layerIndex, paletteColors,'aw');
