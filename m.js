    ...
    try {
        if (totalLayers === 1) {
            ...
            for (let i = 0; i < unlockedButtons.length; i++) {
                ...
                // This is the critical line - ensure a strict positional mapping
            ...
            await new Promise(resolve => {
                resultImg.onload = () => {
                    ...
                    // When updating button colors after processing:
                    colorMappings.forEach(mapping => {
                        // Use the exact target color specified in the mapping
...
function mapFamiliesToMappings(colorFamilies, processedMappings) {
    ...
    colorFamilies.forEach(family => {
        ...
        if (bestMappingIndex !== -1) {
            ...
            // NEW: Determine default mapping for pixels that don't match any family
            let defaultMappingIndex = 0;
            if (processedMappings.length > 0) {
                // Choose default mapping (middle of the list or most prominent)
                defaultMappingIndex = Math.floor(processedMappings.length / 2);
            }
