...
document.addEventListener('DOMContentLoaded', function() {
  ...
  <body>
         <div id="loading-screen" class="fixed inset-0 flex items-center justify-center bg-white bg-opacity-90 z-50" style="position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; display: flex !important; align-items: center !important; justify-content: center !important;">
    ...
    {% for layer in layers %}
      ...
      >
        <canvas 
          ...
          data-original-path="{{ layer.original_path }}"
          data-original-width="{{ layer.original_width }}"
          data-original-height="{{ layer.original_height }}"
          data-display-width="{{ layer.width }}"
          data-display-height="{{ layer.height }}"
...
function exportFile(format = 'tiff') {
    return new Promise((resolve, reject) => {
        ...
        // Collect all layer data for high-resolution export
        layerCanvases.forEach((canvas) => {
            const wrapper = canvas.closest('.layer') || canvas.parentElement;
            const position_top = parseInt(wrapper.style.top) || 0;
            const position_left = parseInt(wrapper.style.left) || 0;
            
            // Get dimensions from data attributes
            const display_width = parseInt(canvas.getAttribute('data-display-width')) || canvas.width;
            const display_height = parseInt(canvas.getAttribute('data-display-height')) || canvas.height;
            ...
            const original_path = canvas.getAttribute('data-original-path');
            const dpi_x = parseInt(canvas.getAttribute('data-dpi-x') || 300);
            const dpi_y = parseInt(canvas.getAttribute('data-dpi-y') || 300);
            const physical_width = parseFloat(canvas.getAttribute('data-original-physical-width-inches') || 0);
            const physical_height = parseFloat(canvas.getAttribute('data-original-physical-height-inches') || 0);
            
            // Calculate scale factors for transforming changes to original resolution
            const scale_x = original_width / display_width;
            const scale_y = original_height / display_height;
            
            const layerData = {
                name: canvas.getAttribute('data-layer-name') || canvas.id,
                position_top: Math.round(position_top * scale_y), // Scale position to original resolution
                position_left: Math.round(position_left * scale_x), // Scale position to original resolution
                display_width: display_width,
                display_height: display_height,
                original_width: original_width,
                original_height: original_height,
                original_path: original_path, // Path to high-res original file
                dpi_x: dpi_x,
                dpi_y: dpi_y,
                physical_width_inches: physical_width,
                physical_height_inches: physical_height,
                scale_x: scale_x,
                scale_y: scale_y,
                // Include current canvas state for any edits made
                current_canvas_data: canvas.toDataURL('image/png', 1.0)
            };
            
            console.log(`Exporting Layer: ${layerData.name}`);
            console.log(`  Original: ${layerData.original_width}x${layerData.original_height}`);
            console.log(`  Display: ${layerData.display_width}x${layerData.display_height}`);
            console.log(`  Scale: ${layerData.scale_x.toFixed(2)}x${layerData.scale_y.toFixed(2)}`);
            console.log(`  Original Path: ${layerData.original_path}`);
            
            layers.push(layerData);
        });
        
        if (layers.length === 0) {
            reject(new Error('No layers found to export'));
            return;
        }
        
        formData.append('layers_data', JSON.stringify(layers));
        formData.append('export_format', format);
        formData.append('use_original_resolution', 'true'); // Flag to use original high-res images
        ...
        .then(blob => {
            ...
            a.download = `exported_highres_${timestamp}.${extensions[format] || 'tif'}`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            a.remove();
